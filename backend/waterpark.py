...
class WaterPark:
    ...
    def become_member(self, name, email, phone_number, birthday, password):
        ...
        birthday = self.format_str_to_date(birthday) 
        ...
        if birthday >= date.today():
            return "Fill the correct birthday."
        if Member.check_password(password) == False or len(password) < 8:
            return "Use password at least 8 characters and only the letters 0-9, a-z, A-Z, (.) or (_)."
    ...
    def show_confirm(self, member_id: int):
        ...
        if member.booking_temp != None:
                member.booking_temp.order = member.order
